...
export async function onRequestPost({ request, env }) {
  ...
  const components = [{
    type: 1,
    components: [
      { type: 2, style: 3, label: 'Accept',             emoji: { name: '✅' }, custom_id: `vdev_accept_${user.id}` },
      { type: 2, style: 1, label: 'Invite to Interview', emoji: { name: '📋' }, custom_id: `vdev_interview_${user.id}` },
